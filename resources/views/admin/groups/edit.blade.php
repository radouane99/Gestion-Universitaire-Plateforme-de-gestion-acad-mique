    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-black text-2xl text-upf-blue leading-tight tracking-tighter italic">
                {{ __("Modifier le Groupe : ") }} {{ $group->name }}
            </h2>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-gradient-to-r from-emerald-500 to-emerald-700 rounded-3xl p-10 text-white shadow-2xl relative overflow-hidden">
                <div class="relative z-10">
                    <h2 class="text-3xl font-black mb-2 italic">Paramètres de la Cohorte</h2>
                    <p class="text-emerald-100 opacity-80">Modifiez le nom ou le niveau académique associé à ce groupe d'étudiants.</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <form method="POST" action="{{ route('admin.groups.update', $group->id) }}" class="p-10 space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $group->name) }}" required class="w-full border-gray-200 rounded-2xl focus:ring-emerald-500 focus:border-emerald-500 p-4 font-bold text-gray-900 bg-gray-50">
                        </div>

                        <!-- Niveau Académique -->
                        <div class="space-y-2">
                            <label for="level" class="text-[10px] font-black uppercase tracking-widest text-gray-400 block">Niveau Académique</label>
                            <input type="text" name="level" id="level" value="{{ old('level', $group->level) }}" required class="w-full border-gray-200 rounded-2xl focus:ring-emerald-500 focus:border-emerald-500 p-4 font-bold text-gray-900 bg-gray-50" placeholder="ex : L1, L2, Master 1">
                        </div>
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <select name="filiere_id" id="filiere_id" required class="w-full border-gray-200 rounded-2xl focus:ring-emerald-500 focus:border-emerald-500 p-4 font-bold text-gray-900 bg-gray-50">
                                <option value="" disabled {{ old('filiere_id', $group->filiere_id) ? '' : 'selected' }}>-- Sélectionner la Filière --</option>
