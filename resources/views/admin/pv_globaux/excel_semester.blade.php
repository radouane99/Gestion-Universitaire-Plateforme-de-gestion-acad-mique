...
<body>
    <table>
        <tr>
            <td colspan="{{ 4 + ($modules->count() * 3) + 3 }}" class="header-title">
                UNIVERSITÉ DE PORTFOLIO (UPF) — PROCÈS-VERBAL SEMESTRIEL
            </td>
        </tr>
        <tr>
            <td colspan="{{ 4 + ($modules->count() * 3) + 3 }}" class="header-subtitle">
                Filière : {{ $filiere->name }} | Semestre : {{ $semester->name }} | Année Académique : {{ $academicYear->name }}
            </td>
        </tr>
        <tr>
            <td colspan="{{ 4 + ($modules->count() * 3) + 3 }}" height="15"></td>
        </tr>
