<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('professor.grades.index') }}" class="w-9 h-9 bg-gray-100 hover:bg-upf-blue hover:text-white rounded-xl flex items-center justify-center text-gray-500 transition-all">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            </a>
            <h2 class="font-black text-2xl text-upf-blue leading-tight tracking-tighter italic">
                Saisie des Notes — {{ $module->name }}
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-8 bg-[#F8FAFC] min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <!-- Hero -->
            <div class="bg-gradient-to-br from-upf-blue via-indigo-700 to-upf-magenta rounded-[2rem] p-8 text-white shadow-2xl relative overflow-hidden">
                <div class="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        <p class="text-blue-200 text-xs font-black uppercase tracking-widest mb-1">Évaluation des Étudiants</p>
                        <h2 class="text-3xl font-black italic leading-tight">{{ $module->name }}</h2>
                        <p class="text-blue-100 text-sm mt-1">Groupe : <strong>{{ $group->name }}</strong> · {{ $group->students->count() }} étudiants</p>
                    </div>
                    <div class="flex gap-3">
                        <div class="text-center bg-white/10 backdrop-blur rounded-2xl px-5 py-3">
                            <p class="text-2xl font-black" id="stat-pass">—</p>
                            <p class="text-[10px] font-black uppercase text-blue-200">Admis</p>
                        </div>
                        <div class="text-center bg-white/10 backdrop-blur rounded-2xl px-5 py-3">
                            <p class="text-2xl font-black" id="stat-fail">—</p>
                            <p class="text-[10px] font-black uppercase text-blue-200">Ajournés</p>
                        </div>
                        <div class="text-center bg-white/10 backdrop-blur rounded-2xl px-5 py-3">
                            <p class="text-2xl font-black" id="stat-avg">—</p>
                            <p class="text-[10px] font-black uppercase text-blue-200">Moy. Classe</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex flex-col md:flex-row md:items-center gap-3 justify-between">
                <div class="flex items-center gap-2 text-sm text-gray-500">
                    <span class="font-black">⌨️ Navigation rapide :</span>
                    <span class="bg-gray-100 rounded-lg px-2 py-0.5 font-mono text-xs font-bold">Tab</span> suivant
                    <span class="bg-gray-100 rounded-lg px-2 py-0.5 font-mono text-xs font-bold">Shift+Tab</span> précédent
                    <span class="bg-gray-100 rounded-lg px-2 py-0.5 font-mono text-xs font-bold">Enter</span> valider ligne
                </div>
            ...
            <div class="bg-white rounded-[2rem] shadow-xl border border-gray-100 overflow-hidden">
                <div class="border-b border-gray-100 px-8 py-5 flex items-center justify-between">
                    <div>
                        <h3 class="font-black text-gray-900 text-lg italic">Registre des Notes</h3>
                        <p class="text-gray-400 text-xs font-semibold">Formule : Moy = ((CC1 + CC2) / 2 × 0.4) + (Exam × 0.6)</p>
                    </div>
                    <div class="flex items-center gap-2 text-xs font-bold">
                        <span class="w-3 h-3 rounded-full bg-emerald-400 inline-block"></span><span class="text-gray-500">≥ 10 Admis</span>
                        <span class="w-3 h-3 rounded-full bg-amber-400 inline-block ml-2"></span><span class="text-gray-500">8–10 Rattrapage</span>
                        <span class="w-3 h-3 rounded-full bg-red-400 inline-block ml-2"></span><span class="text-gray-500">< 8 Ajourné</span>
                    </div>
                </div>
                ...
                <form action="{{ route('professor.grades.store') }}" method="POST" id="grades-form">
                        ...
                        <table class="min-w-full">
                            <thead>
                                <tr class="bg-gray-50 border-b border-gray-100">
                                    ...
                                    <th class="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Étudiant</th>
                                    <th class="px-4 py-4 text-center text-[10px] font-black text-gray-400 uppercase tracking-widest">N° Étu.</th>
                                    <th class="px-4 py-4 text-center text-[10px] font-black text-upf-blue uppercase tracking-widest">CC1 <span class="text-gray-400">/20</span></th>
                                    <th class="px-4 py-4 text-center text-[10px] font-black text-upf-blue uppercase tracking-widest">CC2 <span class="text-gray-400">/20</span></th>
                                    <th class="px-4 py-4 text-center text-[10px] font-black text-upf-magenta uppercase tracking-widest">Examen <span class="text-gray-400">/20</span></th>
                                    <th class="px-6 py-4 text-center text-[10px] font-black text-gray-400 uppercase tracking-widest">Moyenne</th>
                                    <th class="px-4 py-4 text-center text-[10px] font-black text-gray-400 uppercase tracking-widest">Statut</th>
                                </tr>
                            </thead>
                                ...
                                <tr class="hover:bg-blue-50/30 transition-colors duration-150 grade-row" data-index="{{ $index }}">
                                    ...
                                    <td class="px-4 py-4 text-center">
                                        <span class="status-badge text-[10px] font-black uppercase tracking-wide px-2 py-1 rounded-lg" data-row="{{ $index }}">
                                            @if($g?->final_grade)
                                                @if($g->final_grade >= 10)
                                                    <span class="bg-emerald-100 text-emerald-700 px-2 py-1 rounded-lg">✅ Admis</span>
                                                @elseif($g->final_grade >= 8)
                                                    <span class="bg-amber-100 text-amber-700 px-2 py-1 rounded-lg">⚠️ Ratt.</span>
                                                @else
                                                    <span class="bg-red-100 text-red-700 px-2 py-1 rounded-lg">❌ Ajourné</span>
                                                @endif
                                            @else
                    ...
                    <div class="p-6 border-t border-gray-100 bg-gray-50/50 flex flex-col md:flex-row justify-between items-center gap-4">
                        <p class="text-xs text-gray-400 font-semibold">Les moyennes sont calculées automatiquement. Appuyez sur "Enregistrer" pour sauvegarder.</p>
                        <button type="submit" class="px-10 py-4 bg-upf-blue text-white rounded-2xl font-black shadow-xl hover:bg-indigo-700 hover:scale-105 transform transition-all duration-300 flex items-center gap-2">
        ...
        document.getElementById('fill-demo').addEventListener('click', function() {
            document.querySelectorAll('.grade-row').forEach((row, i) => {
                ...
                ['.cc1-input','.cc2-input','.exam-input'].forEach(c => {
                    ...
                    el.classList.add('filled'); el.classList.remove('invalid');
                });
                updateRow(i);
            });
        });
